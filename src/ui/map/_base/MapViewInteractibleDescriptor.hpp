...
class MapViewInteractibleDescriptor : public QWidget {
 ...
 private:
    ...
    void _updateData(const RPZAtom &atom) {
        ...
        if (isNPC) this->_attitudeLbl->setPixmap(QPixmap(atom.descriptiveIconPath()));
            ...
            this->_attitudeLbl->setPixmap(RPZAtom::iconPathByAtomType.value(atomType));
        ...
        if (isNPC) {
            ...
            if (asset) toSet = QPixmap(asset->filepath());
                ...
                toSet = QPixmap(QStringLiteral(u":/assets/default.jpg"));
 ...
 public:
    explicit MapViewInteractibleDescriptor(QWidget *parent = nullptr) : QWidget(parent), _descrLbl(new QLabel(this)), _portraitLbl(new QLabel(this)),
    _attitudeLbl(new QLabel(this)), _anim(new QPropertyAnimation(this, "geometry")) {
