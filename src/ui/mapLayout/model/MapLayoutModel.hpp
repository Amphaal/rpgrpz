        ...
        void handleAlterationRequest(const AlterationPayload* payload) {
            ...
            if (auto mPayload = dynamic_cast<const AtomsWielderPayload*>(payload)) {
                ...
                if (type == Payload::Alteration::Reset) {
                    ...
                    this->endResetModel();
                } else if (type == Payload::Alteration::Added) {  // INSERT
                    // inserts
                    for (auto &atom : mPayload->atoms()) {
                        this->_createAtom(atom);
                    }
                }
            } else if (auto mPayload = dynamic_cast<const RemovedPayload*>(payload)) {  // on remove
            ...
            } else if (auto mPayload = dynamic_cast<const MetadataChangedPayload*>(payload)) {  // on metadata change
            ...
            } else if (auto mPayload = dynamic_cast<const BulkMetadataChangedPayload*>(payload)) {  // on metadata change (bulk)
                LayerMoves categoryMoves;
