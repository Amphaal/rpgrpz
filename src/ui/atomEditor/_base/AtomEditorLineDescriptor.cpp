...
AtomEditorLineDescriptor::AtomEditorLineDescriptor(const RPZAtom::Parameter &paramType, bool supportsBatchEditing)
    : AtomEditorLineDescriptor(_paramDescr.value(paramType), _valSuffix.value(paramType), _icons.value(paramType)) {
...
AtomEditorLineDescriptor::AtomEditorLineDescriptor(const QString &untranslatedDescription, const QString &suffix, const QString &iconPath) :
    _suffix(suffix), _valLbl(new QLabel) {
