...
class GaugeEditor : public SimpleGaugeEditor {
 ...
 public:
    explicit GaugeEditor(const RPZGauge &gauge) : SimpleGaugeEditor({gauge.gaugeValue(), gauge.minGaugeValue(), gauge.maxGaugeValue()}),
        _barNameEdit(new QLineEdit), _colorPicker(new QPushButton), _visibleOnPortraitChk(new QCheckBox) {
            // bar name
